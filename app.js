...
function update(){
 document.getElementById('money').textContent=`💰 $${state.money===Number.MAX_SAFE_INTEGER?'∞':Math.round(state.money).toLocaleString()}`;
 ...
 document.getElementById('vehicles').textContent=state.unlimited?'∞':Object.values(state.vehicles).reduce((a,b)=>a+b,0);
 document.getElementById('atc').textContent=state.unlimited?'∞':state.staff.atc||0;
...
function initMap(a){if(!airportMap)airportMap=L.map('map',{zoomControl:true,preferCanvas:true});airportMap.setView([a.latitude,a.longitude],14);airportLayers.forEach(layer=>airportMap.removeLayer(layer));airportLayers=[];const imagery=L.tileLayer('https://global.imagery.hotosm.org/{z}/{x}/{y}.png',{maxZoom:20,attribution:'© OpenAerialMap / HOT'}).addTo(airportMap);airportLayers.push(imagery);const osm=L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{opacity:.18,maxZoom:19,attribution:'© OpenStreetMap contributors'}).addTo(airportMap);airportLayers.push(osm);}
async function loadOSM(a){const q=`[out:json][timeout:25];(way["aeroway"~"^(runway|taxiway|taxilane)$"](around:8000,${a.latitude},${a.longitude});way["aeroway"="terminal"](around:8000,${a.latitude},${a.longitude});node["aeroway"~"^(gate|parking_position)$"](around:8000,${a.latitude},${a.longitude}););out body geom;`;try{const res=await fetch('https://overpass-api.de/api/interpreter?data='+encodeURIComponent(q));if(!res.ok)throw new Error(`OSM ${res.status}`);const data=await res.json();data.elements.forEach(el=>{const tags=el.tags||{};if(el.type==='way'&&el.geometry?.length){const pts=el.geometry.map(p=>[p.lat,p.lon]);let layer;if(tags.aeroway==='runway')layer=L.polyline(pts,{color:'#e9edf2',weight:10,opacity:.9}).bindTooltip(tags.ref||'Runway');else if(tags.aeroway==='terminal')layer=L.polygon(pts,{color:'#38bdf8',fillColor:'#38bdf8',fillOpacity:.25,weight:2}).bindTooltip(tags.name||'Terminal');else layer=L.polyline(pts,{color:'#f59e0b',weight:4,opacity:.65}).bindTooltip(tags.ref||tags.name||'Taxiway');layer.addTo(airportMap);airportLayers.push(layer);}else if(el.type==='node'&&el.lat&&el.lon){const label=tags.ref||tags.name||(tags.aeroway==='parking_position'?'Parking position':'Gate');const layer=L.circleMarker([el.lat,el.lon],{radius:4,color:'#facc15',fillColor:'#facc15',fillOpacity:.9}).bindTooltip(label);layer.addTo(airportMap);airportLayers.push(layer);}});msg.textContent=`${a.name} loaded from OpenStreetMap + OpenAerialMap imagery.`;}catch(e){msg.textContent=`${a.name} loaded. Live airport geometry unavailable right now.`;console.warn(e);}}
...
function redeemCode(){const input=document.getElementById('storeCode');const code=input.value.trim();if(code==='123123'){Game.activateUnlimited(state);input.value='';msg.textContent='Developer code accepted: unlimited money and resources activated.';update();}else{msg.textContent='Invalid code.';}}
