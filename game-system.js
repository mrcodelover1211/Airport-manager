...
window.AirportManagerGame = (() => {
  ...
  function tick(s){for(const f of s.flights){if(f.status==='departed')continue;f.progress=Math.min(100,f.progress+10);if(f.progress>=100){f.status='departed';if(!s.unlimited)s.money+=f.revenue;s.stats.revenue+=f.revenue;s.stats.passengers+=f.seats;s.stats.completed++;s.reputation=Math.min(100,s.reputation+1)}else f.status=STATUS[Math.min(STATUS.length-2,Math.floor(f.progress/20))];}if(s.stats.completed&&s.stats.completed%10===0)s.level=Math.max(s.level,1+Math.floor(s.stats.completed/10));return s}
