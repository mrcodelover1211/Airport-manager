...
window.AirportManagerGame = (() => {
  ...
  function cost(map,key){return map[key]||5000}
  function canPay(s,n){return s.unlimited||s.money>=n}
  function spend(s,n){if(!s.unlimited){s.money-=n;s.stats.expenses+=n}}
  ...
  function schedule(s,code,flightNumber,gate='A1'){const a=AIRCRAFT[code]||AIRCRAFT.B738;if(!requiredEquipment(s)||(s.staff.ramp||0)<1||(s.staff.dispatcher||0)<1)return[false,'Need pushback, tug, fuel, baggage, catering equipment plus ramp and dispatcher staff'];const f={id:crypto.randomUUID?.()||String(Date.now()),code,flightNumber,gate,status:'arriving',progress:0,seats:a.seats,revenue:a.fee,services:{fuel:false,baggage:false,catering:false,security:false},delay:0};s.flights.push(f);s.stats.aircraft++;return[true,f]}
  function completeServices(s,f){if(!f.services.fuel){f.services.fuel=true;return 'Fuel service completed'}if(!f.services.baggage){f.services.baggage=true;return 'Baggage service completed'}if(!f.services.catering){f.services.catering=true;return 'Catering service completed'}if(!f.services.security){f.services.security=true;return 'Security check completed'}return null}
  function tick(s){for(const f of s.flights){if(f.status==='departed')continue;if(f.delay>0){f.delay--;continue}
      if(f.status==='pushback'){f.progress=Math.min(100,f.progress+10);if(f.progress>=100){f.status='taxiing';f.progress=96}continue}
      if(f.status==='taxiing'){f.progress=Math.min(100,f.progress+10);if(f.progress>=100){f.status='departed';if(!s.unlimited)s.money+=f.revenue;s.stats.revenue+=f.revenue;s.stats.passengers+=Math.round(f.seats*.88);s.stats.completed++;s.reputation=Math.min(100,s.reputation+1)}continue}
      f.progress=Math.min(100,f.progress+10);
      if(f.progress>=100){if(!Object.values(f.services).every(Boolean)){f.progress=90;f.status='servicing';f.delay=1;continue}f.status='ready';f.progress=90;continue}
      if(f.progress<20)f.status='arriving';else if(f.progress<40)f.status='parked';else if(f.progress<70)f.status='servicing';else if(f.progress<90)f.status='boarding';else f.status='ready';
    }if(s.stats.completed>0)s.level=Math.max(s.level,1+Math.floor(s.stats.completed/10));return s}
  function service(s,id){const f=s.flights.find(x=>x.id===id);if(!f||f.status==='departed')return[false,'Flight is not active'];if(f.status==='arriving')return[false,'Aircraft has not reached the stand yet'];const result=completeServices(s,f);if(!result)return[false,'All services already completed'];f.status='servicing';GameEvent(s,'Ground service',`${f.flightNumber}: ${result}.`);return[true,result]}
  function dispatch(s,id){const f=s.flights.find(x=>x.id===id);if(!f||f.status==='departed')return[false,'Flight is not active'];if(!Object.values(f.services).every(Boolean))return[false,'Complete all turnaround services first'];if((s.staff.atc||0)<1)return[false,'ATC is required'];if((s.vehicles.pushback||0)<1)return[false,'A pushback vehicle is required'];f.progress=90;f.status='pushback';return[true,'ATC cleared. Pushback started']}
  function expansionCost(level){return 50000+level*25000}
  function expand(s){const n=expansionCost(s.expansions);if(!canPay(s,n))return[false,n];spend(s,n);s.expansions++;s.level++;GameEvent(s,'Airport expansion','New airport capacity unlocked.');return[true,n]}
  ...
  function acceptContract(s,name,payout,flights=5){s.contracts.push({id:Date.now().toString(),name,payout,target:flights,progress:0,completed:false});return true}
